...
This layer is responsible for data exchange and notification between different execution environments. With ARM TrustZone technology, **SMC(Secure Monitor Call)** is used for notification and normally, a shared memory region will be used for data exchange. If TEE is constructed as a virtual machine by a type 1 hypervisor, the hypervisor should provide notification(e.g. virtual interrupt) and data exchange(e.g. shared pages) mechanism for communicating between the virtual machines of REE and TEE.
...
Before ARMv8 cpu architecture, TrustZone requires a secure monitor firmware that help to do world switch between REE and TEE. For the ARMv8 cpu, the burden is offloaded to [ARM Trusted Firmware](https://github.com/OpenTrustGroup/arm-trusted-firmware/tree/gzos). In here, the secure monitor is a software component in Zircon kernel for dispatching smc requests from REE, non-secure(or REE) IRQ routing and returning to REE when TEE enter idle state; If a REE IRQ occurs but cpu core is in TrustZone mode, TEE should return to REE immediately to minimize the IRQ latency.
...
Starting and activating all virtio devices and their virtio queues. After virtio bus is started, all client services can start to send messages to agents via virtio queue. Before issue this command, .
...
With ARM TrustZone technology enabled, a cpu core execution environment will be separated into two different worlds, one is normal world(REE) and another one is secure world(TEE). The execution time of the cpu core is shared by the two worlds. The nop command is used for REE to yield a cpu core to TEE in order to continue pending tasks or to handle TEE IRQs. It usually happened when a TEE event occurred and the cpu core is still owned by REE, e.g. secure timer interrupt raised.
...
The services are statically configured. It means that the method does not allow a TA to publish a new service to sysmgr in runtime.

- **[Dynamic Service Registration](https://github.com/OpenTrustGroup/garnet/blob/gzos/bin/gzos/sysmgr/dynamic_service_app.cc)**

Due to Trusty IPC allows TA to publish new service in runtime, sysmgr provides the dynamic service registration method for Trusty TA.

Instead of using a global **services.config** for service configuration, a TA should provide a file called **manifest.json**([example](https://github.com/OpenTrustGroup/garnet/blob/gzos/bin/gzos/trusty_app/ipc-unittest/srv/manifest.json)) and list the TA's service names in the **public_services** array in the manifest. Sysmgr will launch the TA when one of it's public services is requested.
...
If a TA publish a service in runtime(after TA is launched) and the service is not defined in public service array,  TA can publish the service to sysmgr via **[service registry interface](https://github.com/OpenTrustGroup/garnet/blob/gzos/public/lib/gzos/sysmgr/fidl/service_registry.fidl)**. The interface not only for a TA to publish service in runtime, but also for a TA to wait on a service that is still not published.

Please note that the dynamic service registration method is only for Trusty TA (since GzOS need to pass the Trusty IPC unit-test). We have realized that it's not secure and this method may be removed in the future.
...
This library implements the Trusty IPC basic components and mechanism for TA to TA and tipc agent to TA communication. GzOS implement the Trusty IPC mechanism based on FIDL interfaces, and FIDL interface implementation is based on Zircon IPC mechanism. In other words, Trusty IPC implementation is based on Zircon IPC mechanism. This is unreasonable and inefficient, but we have no better solution to implement the Trusty IPC, originally implemented in Trusty TEE kernel space, in GzOS userspace.
