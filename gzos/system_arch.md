...
No matter which technology is used, the system should provide isolated and dedicated physical resources (e.g. computing resource, memory, IRQ, I/O ports, timers..., etc.) for TEE to execute trusted applications or host a trusted OS. Also, the system should provide a communication mechanism between REE(Rich Execution Environment, a non-secure computing domain) and TEE. Thus, client applications(CA, executed in REE) can issue commands to trusted applications(TA).

In GzOS design, the communication protocol between REE and TEE is divided into three layers:

![overview](images/system_overview.svg)

### Physical Layer

This layer is responsible for data exchange and notification between different execution environments. With ARM TrustZone technology, **SMC(Secure Monitor Call)** is used for notification and normally a shared memory region will be used for data exchange. If TEE is constructed by virtualization technology, hypervisor should provide notification(e.g. virtual interrupt) and data exchange(e.g. shared pages) mechanism for its managed virtual machines.

### Transport Layer

This layer provides bi-directional message channel for efficiently data transfer between **client service** and **agent**. GzOS uses **Virtio queue** as the transport layer protocol which is widely used in the field of I/O virtualization. Different client services can communicate with it's agent via different virtio queue channels.

### Application Layer
This layer provides high level communication APIs for CA and TA. For example, Trusty TEE provides [Trusty API](https://source.android.com/security/trusty/trusty-ref), OP-TEE provides [GlobalPlatform(GP) client API](https://globalplatform.org/specs-library/tee-client-api-specification/) and QSEE provides [QSEECOM API](https://android.googlesource.com/platform/hardware/qcom/keymaster/+/master/QSEEComAPI.h). With GzOS three layer architecture, developers can implement different high level API to support CA and TA developed for a specific trusted OS. To implement a specific API, developer need to create a pair of **client service** and **agent**. The client service and agent are responsible for creating logic channel(or session) between CA and TA and dispatching messages from CA to TA or from TA to CA.

## System Architecture

After describing the three layer architecture of GzOS, this section will introduce the software components in each layer.
