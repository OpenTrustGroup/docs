...
Due to Trusty IPC allows a TA to publish new service by itself, sysmgr provides the dynamic service registration method for Trusty TA.

If a TA wants to publish a service which is not defined in public service array,  the TA can publish the service to sysmgr via **[service registry interface](https://github.com/OpenTrustGroup/garnet/blob/gzos/public/lib/gzos/sysmgr/fidl/service_registry.fidl)**. In addition to publish service, a TA can also wait on a service that is still not published via the interface.
