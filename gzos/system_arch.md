...
Before discussion the detailed implementation of Trusty API, we need to briefly introduce Fuchsia's application model. Most applications in Fuchsia are managed by [application manager](https://github.com/OpenTrustGroup/garnet/tree/master/bin/appmgr). Applications are placed in a container, called **Environment**. Environments are tree structure, and the environment where appmgr located is called root Environment. User can use the [API](https://github.com/OpenTrustGroup/garnet/tree/gzos/public/lib/app) provided by appmgr to create new Environment and launch new processes in it.

There are two methods to publish service in fuchsia:

1. Parent Environment can provide services to its children. This is called **Environment service**. Processes run in an Environment can use to services provided by its parent Environment

2. Process itself can provide services to its parent. The parent should create a pair of [Zircon channel](https://fuchsia.googlesource.com/zircon/+/master/docs/objects/channel.md), and pass one end to the process while launching it. Then, the parent can use this channel to request services publish from the process. Please note only parent process who holds the other end of the channel can access the services published by the process. Parent process can then send the channel to the other process, make it able to access services.
...
To publish a new service, developers can add an entry in **services.config** which contains service name and the url of the binary that provided the service.  Sysmgr will automatically launch the process when corresponding service is requested. The config file looks like below:
...
Sysmgr will scan all app's public services at initialization time and create proxy services for them. The proxy service will re-direct the service request to corresponding trusted apps (launch it if it's not launched yet). Then, appmgr creates a new Environment called **trusty**, and set it's service root as **trusty's** Environment services. Such that a trusted app created in trusty Environment can request public services defined by other trusted apps. Although tipc agent is not managed by appmgr; but appmgr provides a mechanism for outside processes to access services in sysmgr. Thus, client apps can also access (via tipc agent) services defined by trusty apps.

In order to let trusted apps publish services that is not in the public service array (This is allowed since Trusty API doesn't set restriction on it). We provide additional **[Service Registry](https://github.com/OpenTrustGroup/garnet/blob/gzos/public/lib/gzos/sysmgr/fidl/service_registry.fidl)** to each trusted app. With the Service Registry interface, trusted app can register its local service, and even wait on a specific service to be added to sysmgr.

### [libtrusty_ipc](https://github.com/OpenTrustGroup/garnet/tree/gzos/public/lib/gzos/trusty_ipc)

This module provides basic building blocks that can be used to implement Trusty API. It provides utilities for trusted apps talking with each other via Trusty IPC mechanism. Both Tipc agent and Trusty apps should use this library to talk. The **Service Registry** provided by sysmgr is required to connect **TipcPort** published by other Trusty apps. Please refer to [Trusty API specification]((https://source.android.com/security/trusty/trusty-ref)) for more information.

### [libtrusty_app](https://github.com/OpenTrustGroup/garnet/tree/gzos/public/lib/gzos/trusty_app)

This module implements Trusty API based the building blocks provided by libtrusty_ipc.